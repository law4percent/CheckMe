...
log = get_logger("image_uploader.py")
...
class ImageUploader:
    ...
    def delete_image(self, public_ids: str | list[str], invalidate: bool = True) -> bool:
        ...
        try:
            ...
            if all_deleted:
                log(f"Successfully deleted: {public_ids}", type="info")
                return True
            else:
                log(f"Partial or failed deletion. Result: {result}", type="warning")
                return False
        
        except Exception as e:
            log(f"Failed to delete {public_ids} : {e}", type="error")
